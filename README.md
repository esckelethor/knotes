# K-NOTES ~ K-주석
![LICENSE](https://img.shields.io/badge/All_Rights_Reserved-blue?label=license&labelColor=666&color=1A547E) 
![HTML](https://img.shields.io/badge/HTML%205-black?logo=html5&logoColor=FF6000&labelColor=666&color=FF6000) 
![CSS](https://img.shields.io/badge/CCS_3-black?logo=css&logoColor=0040FF&labelColor=666&color=0040FF) 
![JAVASCRIPT](https://img.shields.io/badge/ECMAScript_15-black?logo=javascript&logoColor=F7DF1E&labelColor=666&color=F7DF1E) 
![VQUERY](https://img.shields.io/badge/Custom_JS_Framework-vQuery-black?labelColor=666&color=5A215A) 
![JSON](https://img.shields.io/badge/JSON-black?logo=json&logoColor=000&labelColor=666&color=444)

## [EN] ABOUT KNOTES ~ K-주석
> **DISCLAIMER:** This project is part of my professional portfolio. The code is available for public inspection but *is not open source*. All rights reserved, and its commercial use or redistribution is strictly prohibited
> 
> **PUBLISHED WEBAPP: [K-NOTES ~ K-주석](https://esckelethor.github.io/knotes/)** *(webapp under construction)*

Initially, a simple webapp was created for studying Korean by collecting notes taken during study sessions

After some time, we decided to add more features, such as:
- Include Spotify with dynamic playlists
- Include a user manual
- Include exercises by topic
- Include exams with timer and score
- Create flashcards for vocabulary
- Include multiple languages:
  - **KR** *(Status: Under construction)*
  - **EN** *(Status: Under construction)*
  - **FR** *(Status: Pending)*

### QUESTIONS?
If you have any questions, please feel free to ask us on:
- LinkedIn: **[yeza](https://www.linkedin.com/in/christian-yezallell-angel-fernandez-736923330/)** / **[yai](https://www.linkedin.com/in/yaiza-priscila-angel-fernandez-aaa9b4146/)**
- GitHub: **[esckelethor](https://github.com/esckelethor)** / **[yaiangel13](https://github.com/yaiangel13)**
- Discord: **esckelethor** / **yaiangel**

## [ES] ACERCA DE KNOTES ~ K-주석
> **AVISO LEGAL:** Este proyecto es parte de mi portafolio profesional. El código está disponible para inspección pública pero *no es de código abierto (open-source)*. Todos los derechos están reservados y su uso comercial o redistribución está estrictamente prohibido
> 
> **WEBAPP PUBLICADA: [K-NOTES ~ K-주석](https://esckelethor.github.io/knotes/)** *(webapp en construcción)*

Al principio se creo una webapp simple para estudiar coreano mediante la recopilación de notas tomadas mientras se estudiaba

Pasado un tiempo hemos decidido darle más funcionalidades, como:
- Incluir Spotify con listas dinamicas
- Incluir un manual de uso
- Incluir ejercicios por tema
- Incluir examenes con temporizador y puntuación
- Creación de tarjetas para estudiar el vocabulario
- Incluir multiidioma:
  - **KR** *(Progreso: En construcción)*
  - **EN** *(Progreso: En construcción)*
  - **FR** *(Progreso: Pendiente)*

### ¿DUDAS?
Si tienes cualquier duda, siente libre de contactarnos en:
- LinkedIn: **[yeza](https://www.linkedin.com/in/christian-yezallell-angel-fernandez-736923330/)** / **[yai](https://www.linkedin.com/in/yaiza-priscila-angel-fernandez-aaa9b4146/)**
- GitHub: **[esckelethor](https://github.com/esckelethor)** / **[yaiangel13](https://github.com/yaiangel13)**
- Discord: **esckelethor** / **yaiangel**
